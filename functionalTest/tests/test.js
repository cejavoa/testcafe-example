...
test('SendText to name input box', async (t) => {
  const test = data.userData.map(async user => (
...
test('Click check boxes and then verify their state', async (t) => {
  const featurePromises = page.featureList.map(async feature => (
    await t // eslint-disable-line no-return-await
      .click(feature.label)
...
data.userData.map(async user => ( // eslint-disable-line no-return-assign
